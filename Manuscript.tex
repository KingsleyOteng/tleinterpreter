...
%	\Input{Five nonnegative integers $M$, $S$, $H$, $B$ and $K$, Two nonnegative floats $X$ and $W$. }
%	\Output{$Y$:$[m, y_0, y_1]$}
	\vspace{0.05in}
	\For{\text{for each item $m$ in $M_m$}}
	{
			\For{\text{for each item $j$ in $Hv_{j,m}$}} 
				{
					\For{\text{for each item $k$ in $Bv_{k,m}$}} 
					{
						 {$Y : [m,h,w] = 0$} \\
						 \For{\text{for each item $\alpha$ in $S_{\alpha}$}}
						 {
						 	\For{\text{for each item $\gamma$ in $K_{\gamma}$}} 
						 	{
						 		\For{\text{for each item $\beta$ in $K_{\beta}$}} 
						 		{
...
\begin{algorithm}
	...
	\Input{weights, $\mathbf{W}$}
	\Output{output features,$\mathbf{Y}$}
	\Output{output features from pooled subsample layer,$\mathbf{Y'}$}
	\vspace{0.1in}
	...
	\For{\text{for each item $m$ in $M_m$}}
	{
		\For{\text{for each item $j$ in $Hv_{j,m}/K$}} 
		{
			\For{\text{for each item $k$ in $Bv_{k,m}/K$}} 
			{
				$Y' : [m,h,w] = 0$
...
\begin{algorithm}
	...
	\SetKwInOut{Output}{Output}
	\Input{weights, $\mathbf{X}$}
	\Output{output features,$\mathbf{X}_{unroll}$}
	\vspace{0.1in}
	...
	\For{\text{each item} $s$ \text{in} $S$} 
	{
		$w_{base} = s\times (K \times K);$ \\
		\For{\text{each item} $p$ \text{in} $K$} 
		{
			\For{\text{each item} $q$ \text{in} $K$} 
			{
			\For{\text{each item} $h$ \text{in} $H'$} 
			{
				\For{\text{each item} $w$ \text{in} $W'$} 
				{
...
%%<<Use this reference https://arnabfly.github.io/arnab_blog/lenet4/>>
...
Backpropagation starts by calculating the gradient of loss function %∂E/∂Y% for the last layer. This process then propagates the gradient from the last layer toward the first layer through all layers of the network. Each layer receives as its input %∂E/∂Y% —“—gradient with respect to its input feature maps (Fig. 16.6).

“We will now describe backpropagation for a convolutional layer, starting with the calculation of %∂E/∂X%.
Note that the calculation of %∂E/∂X% is important for propagating the gradient to the previous layer. The gradient %∂E/∂X% with respect to the channel c of input X is given as the sum of “backward convolution” with corresponding WT(c,m) over all layer outputs m:

“Fig. 16.8 demonstrates the calculation of the %∂E/∂X% function in the form of one matrix for each input feature map. The code assumes that %∂E/∂Y% has been calculated for all the output feature maps of the layer and passed with a pointer argument% dE_dY%. It also assumes that the space of %dE_dX% has been allocated in the device memory whose handle is passed as a pointer argument. The kernel will be generating the elements of% dE_dX%.”

The algorithm for calculating %∂E/∂W% for a convolution layer computation is very similar to that of %∂E/∂X% and is shown in Fig. 16.9. Since each W(c,m) affects all elements of the output Y(m), we should accumulate gradients over all pixels in the corresponding output feature map:
