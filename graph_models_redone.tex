...
%\st{Constraint-based \st{learning algorithms use} algorithms adopt CI tests to construct a \emph{minimal I-map} of network structures called \emph{Complete Partially Directed Acyclic Graph} (CPDAG) which may then be converted into a Bayesian Network: the goal of these algorithms is often to examine the \emh{conditional-independence} (CI) relationships between variables of a network structure, that is the minimal I-map of $P$.}
...
\begin{algorithm}
	\SetKwInOut{Input}{Input}
	\SetKwInOut{Output}{Output}
	\Input{Set of $N$ nodes, random variables $\chi = \{X_i \}_{i=1}^N$, dataset $\mathcal{D} \text{ on } \chi$ and $k$ the upper bound on the number of parents per node} 
	\Output{List of parents per each node; Bayesian network structure $\mathcal{G}$}
	\vspace{0.1in}
i=0:\\
\While{$X_i \leftarrow (i \leq N)$} 
			{
			$\pi_i = \{0\}$\\
...
%%https://www.sciencedirect.com/science/article/pii/S0957417420306278
\begin{equation}
P[G,D] =  f[i,\pi_i ]  = \prod_{j=1}^{\{q_i\}} \dfrac{(r_i-1)!}{(N_{i,j}+r_i-1)!} \prod_{k=1}^{\{r_i\}} \alpha_{i,j,k} !
\end{equation}
%A score-based algorithm is depicted in Figure \ref{score-based-paradigm}\\
